...
@section('content')
    <div class="card mb-3" style="max-width: 18rem;">
        <div class="card-header bg-dark text-white">
            {{$data['essentialoperation']->name}}
        </div>
        <div class="card-body">
            <div class="card-title">
                <h4> {{$data['essentialoperation']->name}}</h4>
            </div>
            <div class="card-text">
                {{$data['essentialoperation']->info}}
            </div>
            ...
            <hr>
            @auth
            <a href="{{'/essentialoperations/'.$data['essentialoperation']->id.'/edit'}}" class="btn btn-primary"> Edit</a>
            <form action="{{route('essentialoperations.destroy',['id'=>$data['essentialoperation']->id])}}" method = "POST">
                @csrf
                @method('DELETE')
